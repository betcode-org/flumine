...
class MarketTest(unittest.TestCase):
    ...
    def test_matches_streaming_market_filter_simulation(self):
        # Actual market id is "1.234"
        self.assertFalse(self.market.matches_streaming_market_filter({"markets": []}))
        self.assertFalse(
            self.market.matches_streaming_market_filter(
                {"markets": ["1.456", "1.1234", "1.2345"]}
            )
        )
        self.assertTrue(
            self.market.matches_streaming_market_filter({"markets": ["acb/def/1.234"]})
        )
        self.assertTrue(
            self.market.matches_streaming_market_filter(
                {"markets": ["acb/def/1.234.json"]}
            )
        )
        self.assertTrue(
            self.market.matches_streaming_market_filter(
                {"markets": ["acb/def/1.234.tar.gz"]}
            )
        )

    def test_matches_streaming_market_filter_live(self):
        mock_market_definition = mock.Mock(
            bsp_market=True,
            betting_type="ODDS",
            event_type_id="7",
            event_id="9876",
        ...
        mock_market_book = mock.Mock(market_definition=mock_market_definition)
        market = Market(mock.Mock(), "1.234", mock_market_book, None)
        for market_filter, is_a_match in [
            # Invalid inputs
            ({"unsupported": True}, False),  # Unsupported field name
            ({"marketIds": ("1,234",)}, False),  # Values must be lists
            # Boolean values
            ({"bspMarket": True}, True),
            ({"bspMarket": False}, False),
            ({"turnInPlayEnabled": True}, True),
            ({"turnInPlayEnabled": False}, False),
            # List values
            ({"marketIds": ["1.234"]}, True),
            ({"marketIds": ["1.456"]}, False),
            ({"bettingTypes": ["ODDS"]}, True),
            ({"bettingTypes": ["ASIAN_HANDICAP"]}, False),
            ({"eventTypeIds": ["7"]}, True),
            ({"eventTypeIds": ["4339"]}, False),
            ({"eventIds": ["9876"]}, True),
            ({"eventIds": ["1234"]}, False),
            ({"marketTypes": ["WIN"]}, True),
            ({"marketTypes": ["PLACE"]}, False),
            ({"venues": ["Ascot"]}, True),
            ({"venues": ["Sheffield"]}, False),
            ({"countryCodes": ["GB"]}, True),
            ({"countryCodes": ["IE"]}, False),
            ({"raceTypes": ["Flat"]}, True),
            ({"raceTypes": ["Hurdle"]}, False),
            ...
            with self.subTest(market_filter=market_filter, is_a_match=is_a_match):
                self.assertEqual(
                    is_a_match,
                    market.matches_streaming_market_filter(market_filter),
                )
